/**
 * Migration script — updates existing database with latest taxonomy
 * and marks the seed scenario as [EXAMPLE] without losing user data.
 *
...
if (!fs.existsSync(DB_PATH)) {
  ...
  console.log('Start the dev server once to create it, or run: npm run dev');
  process.exit(1);
}
...
// ============================================================
// 1. Update taxonomy — upsert by code (insert new, update existing)
// ============================================================
...
const TAXONOMY = [
  ...
  // Loss Event Family (updated - 2 new)
...
// Upsert taxonomy: update existing by code, insert new ones
...
taxonomyTx();
console.log(`Upserted ${TAXONOMY.length} taxonomy entries.`);

// Remove old taxonomy entries that no longer exist
...
if (toRemove.length > 0) {
  ...
  const deleteTx = db.transaction(() => {
    for (const entry of toRemove) {
      deleteStmt.run(entry.code);
    }
  });
  deleteTx();
  console.log(`Removed ${toRemove.length} obsolete taxonomy entries:`, toRemove.map((e) => e.code).join(', '));
} else {
  console.log('No obsolete taxonomy entries to remove.');
}

// ============================================================
// 2. Mark seed scenario as [EXAMPLE] if not already
// ============================================================

const seedScenario = db.prepare("SELECT id, scenario_title FROM scenarios WHERE id = 'SCN-0001'").get() as { id: string; scenario_title: string } | undefined;
if (seedScenario && !seedScenario.scenario_title.startsWith('[EXAMPLE]')) {
  db.prepare("UPDATE scenarios SET scenario_title = ? WHERE id = 'SCN-0001'").run(
    `[EXAMPLE] ${seedScenario.scenario_title}`
  );
  console.log('Marked SCN-0001 as [EXAMPLE].');
} else if (seedScenario) {
  console.log('SCN-0001 already marked as [EXAMPLE].');
} else {
  console.log('SCN-0001 not found (may have been deleted).');
}

// ============================================================
// 3. Update SCN-0001 taxonomy references if it exists
// ============================================================
...
if (seedScenario) {
  ...
  console.log('Updated SCN-0001 taxonomy references.');
}
