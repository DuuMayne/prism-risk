  ...
  try {
    ...
    db.transaction(() => {
      ...
      db.prepare(`INSERT INTO audit_log (action, resource_type, resource_id, actor, ip_address, details)
        VALUES (?, ?, ?, ?, ?, ?)`).run(
        'create', 'scenario', id, null, ip,
        JSON.stringify({ scenario_title: body.scenario_title, scenario_family: body.scenario_family })
      );
    })();

    logger.audit('scenario.create', { resource_id: id, ip, scenario_title: body.scenario_title });

    const scenario = db.prepare('SELECT * FROM scenarios WHERE id = ?').get(id);
    return NextResponse.json(scenario, { status: 201 });
  } catch (err) {
    logger.error('scenario.create.failed', { error: String(err), ip });
